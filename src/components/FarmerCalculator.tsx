...
import { Calculator, Coffee, Sparkles, Coins, Plus, Minus } from "lucide-react";
...
export function FarmerCalculator({ prices }: FarmerCalculatorProps) {
    ...
    return (
        <Card className="w-full border-2 border-primary/30 bg-gradient-to-br from-card via-card to-primary/5 shadow-lg rounded-3xl overflow-hidden">
            <CardHeader className="bg-primary/10 border-b border-primary/20 pb-5 pt-6 px-6 sm:px-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest mb-2">
                            <Sparkles className="h-3 w-3" />
                            Farmer Quick Tool (ರೈತರ ಲೆಕ್ಕಾಚಾರ)
                        </div>
                        <CardTitle className="text-2xl sm:text-3xl font-black text-foreground flex items-center gap-2">
                            <Calculator className="h-7 w-7 text-primary" />
                            Farmer Crop Income Calculator
                        </CardTitle>
                        <p className="text-xs text-muted-foreground font-medium mt-1">
                            Quickly calculate your estimated crop earnings based on today&apos;s live market prices.
                        </p>
                    </div>
                </div>
            </CardHeader>
            ...
            <CardContent className="p-6 sm:p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {[
                                ...
                                >
                                    <span className="block text-xs sm:text-sm">{c.name}</span>
                                    <span className={`block text-[10px] ${selectedCommodity === c.id ? "text-primary-foreground/80" : "text-muted-foreground"}`}>{c.kn}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase tracking-wider text-muted-foreground block">
                            2. Choose District (ಜಿಲ್ಲೆ)
                        </label>
                        <div className="grid grid-cols-2 gap-2">
                            {[
                                { id: "KODAGU", name: "Kodagu Market", kn: "ಕೊಡಗು" },
                                { id: "HASSAN", name: "Hassan Market", kn: "ಹಾಸನ" },
                            ].map((d) => (
                                <button
                                    key={d.id}
                                    type="button"
                                ...
                                >
                                    <span className="block text-xs sm:text-sm">{d.name}</span>
                                    <span className={`block text-[10px] ${selectedDistrict === d.id ? "text-primary-foreground/80" : "text-muted-foreground"}`}>{d.kn}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {/* Quantity Controls */}
                <div className="bg-muted/40 p-5 rounded-2xl border border-border/60 space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div>
                            <label className="text-xs font-black uppercase tracking-wider text-foreground block">
                                3. Quantity ({unitLabel})
                            </label>
                            <span className="text-[11px] text-muted-foreground font-medium">Enter quantity to calculate total income</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="p-2 rounded-xl bg-background border border-border hover:bg-muted text-foreground font-extrabold"
                            >
                    ...
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-border/40">
                        ...
                        {[5, 10, 25, 50, 100].map((q) => (
                            <button
                                ...
                                className="px-3 py-1 text-xs font-extrabold rounded-lg bg-background hover:bg-primary/10 hover:text-primary border border-border/60 transition-colors"
                            >
                ...
                <div className="bg-emerald-500/10 border-2 border-emerald-500/30 p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-3 rounded-xl font-extrabold text-xs uppercase tracking-wider shadow-md">
